/**
 * VHS — vanilla port of Canvas UI's VHS (React).
 * The hero plays back like a worn tape: wave, jitter, head-switching
 * noise, chroma bleed, grain, and scanlines.
 *
 ...
 * plus WebGL2. Anywhere that's missing, the hero is left exactly as
 * authored. prefers-reduced-motion freezes the tape on a still frame.
 * Reuses the .sk-fx-* wrapper CSS from the particle-reveal enhancement.
 */
(function () {
  'use strict';
    ...
    function syncBezelColor() {
      ...
      while (el) {
        ...
        if (bgColor && bgColor !== 'transparent') {
          ...
          bezelCtx.fillStyle = bgColor;
          bezelCtx.fillRect(0, 0, 1, 1);
          var data = bezelCtx.getImageData(0, 0, 1, 1).data;
          if (data[3] > 0) {
